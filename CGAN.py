...
try:
    ...
    print("Error: Module 'jittor' is not installed. Please install it using the following command:")
...
# 优化器部分
optimizer_G = nn.AdamW(generator.parameters(), lr=opt.lr, betas=(opt.b1, opt.b2))
optimizer_D = nn.AdamW(discriminator.parameters(), lr=opt.lr, betas=(opt.b1, opt.b2))
...
number = '28336252826365' #TODO: 写入比赛页面中指定的数字序列（字符串类型）
n_row = len(number)
